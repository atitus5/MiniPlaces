...
class DataLoaderH5(object):
    def __init__(self, **kwargs):
        if "load_size_x" in kwargs:
            self.load_size_x = int(kwargs['load_size_x'])
            self.load_size_y = int(kwargs['load_size_y'])
        else:
            self.load_size_x = int(kwargs['load_size'])
            self.load_size_y = int(kwargs['load_size'])
        ...
        if 'count' in kwargs:
            self.shuffle()
            self.num = kwargs["count"]
            self.im_set = np.array(f['images'])[:self.num]
            self.lab_set = np.array(f['labels'])[:self.num]
        


        assert self.im_set.shape[0]==self.lab_set.shape[0], '#images and #labels do not match!'
        assert self.im_set.shape[2]==self.load_size_x, 'Image size error!'
        assert self.im_set.shape[1]==self.load_size_y, 'Image size error!'
    ...
    def next_batch(self, batch_size):
        ...
        images_batch = np.zeros((batch_size, self.load_size_y, self.load_size_x, 3)) 
