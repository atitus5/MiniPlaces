...
path_save = './saved_models/vgg_B_bn'
start_from = ''
...
weights = {
    ...
    'wf11': tf.Variable(tf.random_normal([8192, 4096], stddev=np.sqrt(2./8192))),
    'wf12': tf.Variable(tf.random_normal([4096, 4096], stddev=np.sqrt(2./4096))),
...
biases = {
    ...
    'bf11': tf.Variable(tf.zeros(4096)),
    'bf12': tf.Variable(tf.zeros(4096)),
    'bo': tf.Variable(tf.zeros(100))
}

def vgg_B(x, keep_dropout, train_phase):
    ...
    # Conv3-512 + ReLU + Conv3-512 + ReLU + 2x2 Pool
    conv9 = tf.nn.conv2d(pool4, weights['wc9'], strides=[1, 1, 1, 1], padding='SAME')
    conv9 = batch_norm_layer(conv9, train_phase, 'bn9')
    conv9 = tf.nn.relu(tf.nn.bias_add(conv9, biases['bc9']))
    conv10 = tf.nn.conv2d(conv9, weights['wc9'], strides=[1, 1, 1, 1], padding='SAME')
    conv10 = batch_norm_layer(conv10, train_phase, 'bn10')
    conv10 = tf.nn.relu(tf.nn.bias_add(conv10, biases['bc10']))
    pool5 = tf.nn.max_pool(conv10, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')

    # FC + ReLU + Dropout
    fc11 = tf.reshape(pool5, [-1, weights['wf11'].get_shape().as_list()[0]])
    fc11 = tf.add(tf.matmul(fc11, weights['wf11']), biases['bf11'])
    fc11 = batch_norm_layer(fc11, train_phase, 'bn11')
    fc11 = tf.nn.relu(fc11)
    fc11 = tf.nn.dropout(fc11, keep_dropout)
    
    # FC + ReLU + Dropout
    fc12 = tf.add(tf.matmul(fc11, weights['wf12']), biases['bf12'])
    fc12 = batch_norm_layer(fc12, train_phase, 'bn12')
    fc12 = tf.nn.relu(fc12)
    fc12 = tf.nn.dropout(fc12, keep_dropout)

    # Output FC
    out = tf.add(tf.matmul(fc12, weights['wo']), biases['bo'])
...
# Construct model
logits = vgg_B(x, keep_dropout, train_phase)
