...
path_save = './saved_models/vgg_D_bn'
start_from = ''
...
weights = {
    ...
    'wf14': tf.Variable(tf.random_normal([8192, 4096], stddev=np.sqrt(2./8192))),
    'wf15': tf.Variable(tf.random_normal([4096, 4096], stddev=np.sqrt(2./4096))),
...
biases = {
    ...
    'bf14': tf.Variable(tf.zeros(4096)),
    'bf15': tf.Variable(tf.zeros(4096)),
    'bo': tf.Variable(tf.zeros(100))
}

def vgg_D(x, keep_dropout, train_phase):
    ...
    # Conv3-256 + ReLU + Conv3-256 + ReLU + Conv3-256 + ReLU + 2x2 Pool
    ...
    # Conv3-512 + ReLU + Conv3-512 + ReLU + Conv3-512 + ReLU + 2x2 Pool
    ...
    # FC + ReLU + Dropout
    fc14 = tf.reshape(pool5, [-1, weights['wf14'].get_shape().as_list()[0]])
    fc14 = tf.add(tf.matmul(fc14, weights['wf14']), biases['bf14'])
    fc14 = batch_norm_layer(fc14, train_phase, 'bn14')
    fc14 = tf.nn.relu(fc14)
    fc14 = tf.nn.dropout(fc14, keep_dropout)
    
    # FC + ReLU + Dropout
    fc15 = tf.add(tf.matmul(fc14, weights['wf15']), biases['bf15'])
...
# Construct model
logits = vgg_D(x, keep_dropout, train_phase)
